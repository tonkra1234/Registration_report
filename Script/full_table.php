...
if(!empty($search_key)){
    $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `full_track` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%'");
    $total_records = mysqli_fetch_array($result_count);
    $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; 

    $result_full = mysqli_query($conn,"SELECT * FROM `full_track` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%' ORDER BY date_fast DESC LIMIT $offset, $total_records_per_page");    
}
else{
if ($month_year === '') {
    if ($Status === '') {
    ...
    <div class="col-sm p-3 min-vh-100">
        ...
        <div class="container-fluid p-5">
            <form class="my-lg-3 my-2 shadow p-lg-3 p-2" action="" method="GET" accept-charset="utf-8">
                <div class="row">
                    ...
                    <div class="col-sm-2 col-12 d-grid">
                        <a class="btn btn-danger px-5 ms-1" role="button" href="./full_table.php">Clear all</a>
                    </div>
                </div>
            </form>
            <div class="card shadow">
                <div class="card-header d-flex justify-content-between">
                    <div class="my-1">
                        <h4 class="pt-1" style="color: #326292 ;">Table of Full dossier</h4>
                    </div>
                </div>
        ...
        ?>
                            <tr>
                                ...
                                <?php else : ?>
                                <span
                                    class="badge bg-warning text-dark"><?php echo (array_key_exists("Show_status",$row_full)) ? $row_full["Show_status"] : "" ; ?></span>
                                </td>
                                <?php endif; ?></td>
                                <td class="text-center"><a type="button" class="btn btn-primary"
                                        href="./detail.php?id=<?php echo $row_full["id"];?>&type=full_track">Click</a>
                                </td>
                                <td class="text-center"><a type="button" class="btn btn-success"
                                        href="./report_show.php?id=<?php echo $row_full["id"];?>&type=full_track">Click</a>
                                </td>
                            </tr>
                            <?php
            $i++;
