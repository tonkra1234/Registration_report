...
if(!empty($search_key)){
    $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `fast_track` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%'");
    $total_records = mysqli_fetch_array($result_count);
    $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; 

    $result_fast = mysqli_query($conn,"SELECT * FROM `fast_track` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%' ORDER BY Date_fast DESC LIMIT $offset, $total_records_per_page");    
}
else{
    if ($month_year === '') {
        if ($Status === '') {
