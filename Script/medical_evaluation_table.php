...
if(!empty($search_key)){
    $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `medical_renewal` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%'");
    $total_records = mysqli_fetch_array($result_count);
    $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; 

    $result_medical_renewal = mysqli_query($conn,"SELECT * FROM `medical_renewal` WHERE CONCAT(Dossier_ID,Generic_Name,Brand_Name) LIKE '%$search_key%' ORDER BY date_fast DESC LIMIT $offset, $total_records_per_page");    
}
else{
    if ($month_year === '') {
        if ($Status === '') {
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `medical_renewal` WHERE `Show_status` = '$Status' ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; 
    
            $result_medical_renewal = mysqli_query($conn,"SELECT * FROM `medical_renewal` WHERE `Show_status` = '$Status' ORDER BY date_fast DESC LIMIT $offset, $total_records_per_page");
        }
        ...
        if($Status === ''){
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `medical_renewal` WHERE MONTH(date_fast) = $month and Year(date_fast) = $year AND `Show_status` = '$Status'");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; 
    
            $result_medical_renewal = mysqli_query($conn,"SELECT * FROM `medical_renewal` WHERE MONTH(date_fast) = $month and Year(date_fast) = $year AND `Show_status` = '$Status' ORDER BY date_fast DESC LIMIT $offset, $total_records_per_page");
        }
    }
}
    ...
    <div class="col-sm p-3 min-vh-100">
        ...
        ?>
                            <tr>
                                ...
                                <td><?php echo $row_medical_renewal["Brand_name"]; ?></td>
                                <td><?php echo $row_medical_renewal["Missing_document"]; ?></td>
